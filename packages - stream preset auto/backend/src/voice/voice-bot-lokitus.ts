...
async function resolveVideoUrl(url: string, maxHeight: number = 720): Promise<string> {
  ...
  // UUSI LOGIIKKA: Jos maxHeight on 0 (auto), käytetään 2160p (4K) rajaa.
  // Muuten käytetään käyttäjän/koodin antamaa rajaa.
  const targetHeight = maxHeight === 0 ? 2160 : maxHeight;

  // 1. Pyydetään erilliset bestvideo ja bestaudio, yhdistetään ne + merkillä
  // HUOM: Tässä käytetään nyt muuttujaa ${targetHeight} eikä maxHeight!
  const formatFilter = `bestvideo[height<=${targetHeight}][dynamic_range=SDR]+bestaudio/best[height<=${targetHeight}][dynamic_range=SDR]/best`;
...
export class VoiceBot extends EventEmitter {
  ...
  constructor(config: VoiceBotConfig) {
    ...
    // --- LISÄÄ TÄSTÄ ALAS ---
    // TS6-SPY: Salakuunnellaan kaikki raakaliikenne, joka liittyy striimeihin!
    this.client.on('raw', (line: string) => {
      if (line && line.toLowerCase().includes('stream')) {
        console.log(`[TS6-MANAGER RAW IN] ${line}`);
      }
    });

    this.client.on('send', (line: string) => {
      if (line && line.toLowerCase().includes('stream')) {
        console.log(`[TS6-MANAGER RAW OUT] ${line}`);
      }
    });
    // --- LISÄÄ TÄHÄN ASTI ---
