...
export class StreamSignaling extends EventEmitter {
  ...
  private handleCommand(parsed: any): void {
    // --- LISÄÄ TÄMÄ RIVI ---
    if (parsed.name && parsed.name.toLowerCase().includes('stream')) {
        console.log(`[TS6-MANAGER PARSED IN] Cmd: ${parsed.name} | Params:`, JSON.stringify(parsed.params || {}));
    }
    // -----------------------
    switch (parsed.name) {
  ...
  sendSetupStream(params: {
    ...
    this.client.sendCommand(buildCommand('setupstream', {
      ...
      accessibility: '0', // OHITETAAN MUUT KÄSKYT JA PAKOTETAAN PUBLIC
  ...
  sendJoinResponse(viewerClid: number, streamId: string, accept: boolean = true, offerSdp?: string): void {
    // --- LISÄÄ TÄMÄ ---
    console.log(`[TS6-MANAGER PARSED OUT] sendJoinResponse clid=${viewerClid}, streamId=${streamId}, accept=${accept}`);
    // ------------------
