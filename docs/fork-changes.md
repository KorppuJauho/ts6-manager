...
`joinstreamrequest` for any stream another client runs in view, logs the SDP
...
remaining thing to compare against. webspeak3 recovered the command from
`TeamSpeak.dll`; its full parameter set (`muted volume hidden` too) and the
`requeststreaminfo` query that finds streams already running come from
[WebSpeak](https://github.com/EchoSixHIYA/WebSpeak-client-for-TeamSpeak), whose
browser viewers watch real TS6 clients' streams. The query goes only to
clients in the bot's own channel, to stay clear of anti-flood.
